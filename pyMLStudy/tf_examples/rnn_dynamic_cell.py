...
def main():
    ...
    rnn_input = tf.one_hot(x, num_class)
    # rnn_input = tf.unstack(x_one_hot, axis=1)
    cell = tf.contrib.rnn.BasicRNNCell(state_size)
    # rnn_outputs: (batch_size,num_steps,state_size)
    rnn_outputs, final_state = tf.nn.dynamic_rnn(cell, rnn_input, initial_state=init)
    ...
    logits = tf.reshape(tf.matmul(tf.reshape(rnn_outputs, [-1, state_size]), w) + b, [batch_size, num_steps, num_class])
