...
class Regressor(object):
    ...
    def regression(self):
        x_train, x_test, y_train, y_test = train_test_split(self._x, self._y, train_size=self._trainSize,
                                                            random_state=1)
        ...
        if self._regType == RegType.LinearReg:
            ...
            self._model = Pipeline([('ss', StandardScaler()),
                                    ...
                                    ('reg',
                                     ElasticNetCV(l1_ratio=[0.1, 0.3, 0.5, 0.7, 0.99, 1], alphas=np.logspace(-3, 2, 5),
...
def mainAdvertising():
    data = load_advertising_data(_pathAdvertisingData)
    plot_advertising_data()
...
def mainBoston():
    data = load_boston_data(_pathHousingData)
    y, x = data.label_feature
